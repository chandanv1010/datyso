@section('headerTitle', 'Quản trị - Danh sách bài viết')
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="form-check form-switch ms-4 d-flex justify-content-center"
                                    ...
                                    <form action="{{ route('admin.post.status', ['id' => $post->id]) }}" method="POST"
                                        id="form-status-{{ $post->id }}">
                                        @csrf
                                        <input type="checkbox"
                                            class="form-check-input status {{ $post->status == 1 ? 'active-item' : 'deactive-item' }}"
                                            name="status" data-value="{{ $post->id }}" id="status-{{ $post->id }}"
                                            {{ $post->status == 1 ? 'checked' : '' }}>
                                        <label class="form-check-label" type="hidden"
                                            for="status-{{ $post->id }}"></label>
                                    </form>
                                </div>
                            </td>
                            <td class="text-center">
